...
@Component({
  ...
  `,
  styleUrls: ['./net-energy-consumption-graph.component.css']
})
export class NetEnergyConsumptionGraphComponent implements AfterViewInit, OnDestroy, OnChanges {
  ...
  updateChart() {
    ...
    monthlyData.forEach(entry => {
      if (!yearData[entry.year.toString()]) {
        yearData[entry.year.toString()] = { netConsumption: Array(12).fill(0) };
      }
      
      if (entry.month && entry.netConsumption !== undefined) {
        const monthIndex = entry.month - 1;
        yearData[entry.year.toString()].netConsumption[monthIndex] = entry.netConsumption;
      }
    });
    ...
    const datasets = Object.keys(yearData)
      ...
      .map((year, index) => {
        ...
        return {
          label: `Year ${year}`,
          data: yearData[year].netConsumption,
  ...
  processBillsForMonthlyData(bills: ElectricBill[]) {
    ...
    bills.forEach(bill => {
      ...
      if (bill.serviceYear >= threeYearsAgo && bill.serviceYear <= currentYear) {
        ...
        if (!monthlyTotals[key]) {
          monthlyTotals[key] = { consumption: 0, sentBack: 0 };
        }
        
        if (bill.consumptionKwh !== undefined) {
          monthlyTotals[key].consumption += bill.consumptionKwh;
        }
        if (bill.sentBackKwh !== undefined && bill.sentBackKwh !== null) {
          monthlyTotals[key].sentBack += bill.sentBackKwh;
        }
      }
    });
    ...
    for (let year = threeYearsAgo; year <= currentYear; year++) {
      for (let month = 1; month <= 12; month++) {
        ...
        const totals = monthlyTotals[key] || { consumption: 0, sentBack: 0 };
        const netConsumption = totals.consumption - totals.sentBack;
