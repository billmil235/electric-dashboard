import { Component, ViewChild, ElementRef, AfterViewInit, OnDestroy, input, signal, SimpleChanges, effect, ChangeDetectionStrategy } from '@angular/core';
...
@Component({
  ...
  `,
  styleUrls: ['./monthly-sent-back-graph.component.css']
})
export class MonthlySentBackGraphComponent implements AfterViewInit, OnDestroy {
  ...
  updateChart() {
    ...
    monthlyData.forEach(entry => {
      if (!yearData[entry.year.toString()]) {
        yearData[entry.year.toString()] = { sentBack: Array(12).fill(0) };
      }

      if (entry.month && entry.sentBack !== undefined) {
        const monthIndex = entry.month - 1;
        yearData[entry.year.toString()].sentBack[monthIndex] = entry.sentBack;
      }
    });
  ...
  processBillsForMonthlyData(bills: ElectricBill[]) {
    ...
    bills.forEach(bill => {
      ...
      if (bill.serviceYear >= threeYearsAgo && bill.serviceYear <= currentYear) {
        ...
        monthlyTotals[key] += bill.sentBackKwh;
      }
    });
    ...
    for (let year = threeYearsAgo; year <= currentYear; year++) {
      for (let month = 1; month <= 12; month++) {
        ...
        const sentBack = monthlyTotals[key] || 0;
