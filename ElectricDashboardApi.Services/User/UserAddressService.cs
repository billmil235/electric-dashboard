    ...
    IAddServiceAddressCommand addServiceAddressCommand) : IUserAddressService
{
    ...
    public async ValueTask<IReadOnlyCollection<ServiceAddress>> GetServiceAddresses(Guid userGuid)
    {
        ...
        async ValueTask<IReadOnlyCollection<ServiceAddress>> Factory(CancellationToken cancel) =>
            await getUserAddressesQuery.GetUserAddresses(userGuid);
    }

    public async ValueTask<ServiceAddress?> AddAddress(Guid userGuid, ServiceAddressDto serviceAddress)
    {
